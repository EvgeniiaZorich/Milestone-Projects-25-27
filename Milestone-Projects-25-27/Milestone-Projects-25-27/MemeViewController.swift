...
//
//  Created by Евгения Зорич on 13.04.2023.
//
...
class MemeViewController: UIViewController {
    ...
    func createMemeSecond() {
        let ac = UIAlertController(title: "Write a text for the bottom of the meme", message: nil, preferredStyle: .alert)
        ac.addTextField()
    ...
    func createMeme(action: UIAlertAction) {
        ...
        let ac = UIAlertController(title: "Write a text for the top of the meme", message: nil, preferredStyle: .alert)
        ac.addTextField()
        ac.addAction(UIAlertAction(title: "Ok", style: .default) { [self]_ in
    ...
    func createTopMeme(text: String) {
        let width = imageView.image?.size.width ?? 200
        let height = imageView.image?.size.height ?? 200
        ...
        let img = renderer.image { ctx in
            ...
            let attrs: [NSAttributedString.Key: Any] = [
                .font: UIFont.systemFont(ofSize: 20), .backgroundColor: UIColor.white,
    ...
    func createBottomMeme(text: String) {
        ...
        let img = renderer.image { ctx in
            ...
            let attrs: [NSAttributedString.Key: Any] = [
                .font: UIFont.systemFont(ofSize: 30), .backgroundColor: UIColor.white,
                .paragraphStyle: paragraphStyle
            ]
